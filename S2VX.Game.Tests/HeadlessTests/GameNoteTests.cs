...
namespace S2VX.Game.Tests.HeadlessTests {
    ...
    public class GameNoteTests : S2VXTestScene {
        ...
        public void OnPress_StackedNotes_HitsTopNote() {
            ...
            AddStep("Seek clock", () => Stopwatch.Seek(Story.Notes.HitThreshold + 20));
            AddAssert("Hit only top note", () => PlayScreen.ScoreProcessor.Score == Story.Notes.HitThreshold * 2 + 25);
        }
        ...
        public void OnPress_NoteThenNote_HitsLaterNote() {
            ...
            AddStep("Hold key", () => InputManager.PressKey(Key.Z));
            AddStep("Release key", () => InputManager.ReleaseKey(Key.Z));
            AddStep("Seek clock", () => Stopwatch.Seek(Story.Notes.HitThreshold + 10));
            AddAssert("Hit only top note", () => PlayScreen.ScoreProcessor.Score == Story.Notes.HitThreshold);
        }

        [Test]
        public void OnPress_OutsideMissThreshold_DoesNothing() {
            AddStep("Add note", () => Story.AddNote(new GameNote { HitTime = Story.Notes.HitThreshold + 50 }));
            AddStep("Move mouse to note", () => InputManager.MoveMouseTo(Story.Notes.Children.First()));
            AddStep("Hold key", () => InputManager.PressKey(Key.Z));
            AddStep("Release key", () => InputManager.ReleaseKey(Key.Z));
            AddStep("Seek after post-threshold", () => Stopwatch.Seek(Story.Notes.HitThreshold * 2 + 60));
            AddAssert("Note was missed", () => PlayScreen.ScoreProcessor.Score == Story.Notes.HitThreshold);
        }

        [Test]
        public void OnPress_WithinMissThreshold_RegistersHit() {
            GameNote note = null;
            AddStep("Add note", () => Story.AddNote(note = new GameNote { HitTime = Story.Notes.HitThreshold + 50 }));
            AddStep("Seek between pre-threshold and HitTime", () => Stopwatch.Seek(note.HitTime - Story.Notes.HitThreshold / 2));
            AddStep("Move mouse to note", () => InputManager.MoveMouseTo(Story.Notes.Children.First()));
            AddStep("Hold key", () => InputManager.PressKey(Key.Z));
            AddStep("Release key", () => InputManager.ReleaseKey(Key.Z));
            AddStep("Seek after post-threshold", () => Stopwatch.Seek(Story.Notes.HitThreshold * 2 + 60));
            AddAssert("Note was hit", () => PlayScreen.ScoreProcessor.Score == Story.Notes.HitThreshold / 2);
        }
    }
}
